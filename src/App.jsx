...
export default function App() {
  ...
  // ── Escritura: state + localStorage + Firebase ────────────────────────────
  ...
  const addLog = useCallback((tipo, det, extra = '') => {
    setHist(prev => {
      const next = [...prev, { id: uid(), ts: now(), tipo, usuarioId: usuRef.current, det, extra }]
      lsSet('oc_hist', next)
  ...
  useEffect(() => {
    ...
    if (to !== lsGet('oc_tarea_obra', SEED_TAREAS_OBRA)) writeTObra(to)
    if (tg !== lsGet('oc_tarea_gral', SEED_TAREAS_GENERALES)) writeTGral(tg)
  }, [])

  // ── cambiarEstado ─────────────────────────────────────────────────────────
  const cambiarEstado = useCallback((id, nuevoEstado, comentario, esGral = false) => {
    const hacer = (arr, writer) => writer(arr.map(t => t.id !== id ? t : {
      ...t, estadoActual: nuevoEstado,
      changelog: [...t.changelog, { id: uid(), estado: nuevoEstado, usuarioId: usuRef.current, ts: now(), comentario: comentario || '' }],
    }))
  ...
  const vencidasObra = tareasObra.filter(t => t.obraId === obraActiva && t.estadoActual === 'vencida').length
